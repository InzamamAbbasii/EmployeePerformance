...
const EvaluatedTeacherByKpi = ({ navigation, route }) => {
  ...
  const getEvaluationResult = () => {
    ...
    setData([]);
    var InsertApiURL1 = `http://${ip}/EmpPerformanceApi/api/Evaluation/getEvaluationResult2?empNo=${route.params.Id}&staff=${selectedStaff}`;
    fetch(InsertApiURL1,
      {
        method: 'GET',
      }
        ...
        if (response == "false") {
          ...
          response.forEach(element => {
            setData(data => [...data,
            {
              ...
              AcademicPercentage: element.AcademicPercentage,
              AdministrationPercentage: element.AdministrationPercentage,
              ProjectPercentage: element.ProjectPercentage,
              AveragePercentage: element.AveragePercentage,
            }
            ])
          });
        }
...
export default EvaluatedTeacherByKpi;